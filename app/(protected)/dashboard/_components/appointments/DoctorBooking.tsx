...
/**
 * DoctorBooking — Steps 2 → 3 → 4 → 5 of the appointment booking flow.
 *
 * Step 2 (no doctorId)                        : Pick a doctor.
 * Step 3 (doctorId, no serviceId)             : Pick a service (determines slot duration).
 * Step 4 (doctorId + serviceId, no slotId)    : Pick a date & time (filtered by service).
 * Step 5 (doctorId + serviceId + slotId)      : Review, add notes, confirm.
 */
...
// ── Step indicator ─────────────────────────────────────────────────────────────
function StepIndicator({ step }: { step: 2 | 3 | 4 | 5 }) {
  const steps = [
    { n: 2, label: 'Doctor' },
    { n: 3, label: 'Service' },
    { n: 4, label: 'Date & Time' },
...
export default function DoctorBooking({
  userId, clinicId, clinicName, doctorId, serviceId, slotId,
  onSelectDoctor, onSelectService, onSelectSlot, onBooked,
}: Props) {
  ...
  // Slots are already filtered by service_id on fetch — also keep client-side
  // duration filter as safety net for any legacy slots with NULL service_id
  const serviceDuration = selectedService?.duration_minutes ?? 0
  const compatibleSlots = serviceDuration > 0
    ? slots.filter((slot) => {
        const dur = parseTimeToMinutes(slot.end_time) - parseTimeToMinutes(slot.start_time)
        return dur >= serviceDuration
      })
    : slots
  ...
  const fetchSlots = useCallback(async (dId: string, svcId: string) => {
    ...
    try {
      ...
      // Filter by service_id — matches slots created for this specific service
      query = query.eq('service_id', svcId)

      const { data, error } = await query
      if (error) throw error
      // Only show slots that still have availability
      if (data) setSlots(data.filter((s: { current_bookings: number; max_bookings: number }) => s.current_bookings < s.max_bookings))
    } catch (err) {
  ...
  const fetchServices = useCallback(async (dId: string) => {
    ...
    try {
      ...
      const { data, error } = await (supabase as any)
        ...
        .select('service_id, services(id, name, description, price, duration_minutes)')
        .eq('doctor_id', dId)
        .eq('is_active', true)
      if (error) throw error
      if (data) {
        // Flatten and deduplicate by service name
  ...
  useEffect(() => {
    if (doctorId) fetchServices(doctorId)
  ...
  const handleConfirm = async () => {
    ...
    try {
      ...
      // Use the book-appointment edge function which handles:
      // - Atomic slot reservation (race-condition safe)
      // - Patient profile data population
      ...
      if (data && !data.success) {
        ...
        // Handle slot-no-longer-available specifically
  ...
  const step = !doctorId ? 2 : !serviceId ? 3 : !slotId ? 4 : 5
    ...
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
      ...
      <div className="lg:col-span-2 space-y-4">
        ...
        <div className="bg-white rounded-2xl border border-lhc-border px-6 py-5 shadow-sm">
          ...
          <StepIndicator step={step as 2 | 3 | 4 | 5} />
        </div>
              ...
              {loadingDoctors ? (
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  {doctors.map((doc) => (
                    <div
                      key={doc.id}
                      className="group relative rounded-2xl border-2 border-lhc-border hover:border-lhc-primary bg-lhc-background/30 hover:bg-white p-4 transition-all duration-150 hover:shadow-sm"
                    >
                      {/* Favorite button */}
                      ...
                      <button
                        ...
                        className="text-left w-full focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-lhc-primary rounded-xl"
                      >
              ...
              {loadingServices ? (
                ...
                <div className="space-y-2.5">
                  {services.map((svc) => (
                    <button
                      key={svc.id}
                      onClick={() => onSelectService(svc.id, svc.name)}
              ...
              <div className="p-6">
                <dl className="space-y-0">
                  {[
                    ...
                    ...(selectedService ? [{ label: 'Service', value: `${selectedService.name}${selectedService.duration_minutes ? ` · ${selectedService.duration_minutes} min` : ''}` }] : []),
            ...
            {selectedService && (
              <div className="flex items-center gap-2 text-xs text-lhc-text-muted pt-1 border-t border-lhc-border/60">
                ...
                {selectedService.duration_minutes && (
                  <span className="text-lhc-text-muted/60">({selectedService.duration_minutes} min)</span>
                )}
              </div>
            )}
            {selectedDate && (
